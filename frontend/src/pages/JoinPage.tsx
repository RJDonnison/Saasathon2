...
export default function JoinPage() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-canvas text-ink" id="top">
      ...
      <div className="mx-auto grid w-full max-w-6xl flex-1 items-center gap-10 px-4 py-6 sm:px-6 lg:grid-cols-[1fr_1.05fr] lg:gap-16 lg:py-12">
        <section className="flex flex-col gap-5">
          <h1 className="m-0! max-w-xl font-display! text-[length:clamp(36px,5vw,56px)]! leading-[1.08]! font-semibold! tracking-[-0.035em]!">
            A shared space for the{" "}
            <span className="text-mint-ink">whole class.</span>
          </h1>
        ...
        <div className="rounded-[28px] bg-mint p-4 sm:p-10 lg:p-12">
          <section
            className={`mx-auto flex w-full max-w-md flex-col gap-5 p-6 sm:p-8 ${CARD}`}
            id="get-started"
            aria-label="Sign in"
          >
            {!session ? (
                ...
                {role === "student" ? (
                  <>
                    <div className="flex flex-col gap-1.5">
                      ...
                      <p className="m-0 text-sm text-muted">
                        Ask your teacher to invite this email address.
                        Invitations show up here, and you choose whether to
                        join.
                      </p>
                    </div>
                    <InvitationList />
      ...
      <div className="mt-12 bg-forest text-canvas lg:mt-16">
        <section
          ...
          <div className="grid gap-10 md:grid-cols-3 md:gap-12">
            {POINTS.map((point, i) => (
              <div key={point.title} className="flex flex-col gap-3">
                ...
                <h2 className="m-0! font-display! text-[length:22px]! leading-[1.2]! font-semibold! tracking-[-0.02em]!">
                  {point.title}
                </h2>
                <p className="m-0 text-sm leading-relaxed text-canvas/70">
                  {point.body}
                </p>
              </div>
            ))}
        ...
        <footer className="mx-auto w-full max-w-6xl px-4 sm:px-6">
          <div className="flex h-16 items-center justify-between border-t border-canvas/15 text-xs text-canvas/55">
            <span>© 2026 Loop</span>
            <a className="hover:text-canvas" href="#top">
              Back to top
            </a>
          </div>
        </footer>
