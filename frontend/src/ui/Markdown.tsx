...
const components: Components = {
  ...
  h3: ({ children }) => (
    <h4 className="m-0 text-[15px] font-semibold text-ink font-display">
      {children}
    </h4>
  ),
  h4: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="m-0 flex list-disc flex-col gap-1.5 pl-5">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="m-0 flex list-decimal flex-col gap-1.5 pl-5">{children}</ol>
  ),
  strong: ({ children }) => (
  ...
  blockquote: ({ children }) => (
    <blockquote className="m-0 border-l-2 border-accent pl-4 text-muted">
      {children}
    </blockquote>
  ),
  pre: ({ children }) => (
    <pre className="m-0 overflow-x-auto rounded-xl bg-ink px-4 py-3.5 text-[13px] leading-6 text-surface-soft font-mono">
      {children}
    </pre>
  ),
  ...
  th: ({ children }) => (
    <th className="border-b border-border bg-surface-soft px-3 py-2 font-semibold text-ink">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="border-b border-border px-3 py-2 [tr:last-child_&]:border-b-0">
      {children}
    </td>
  ),
};
