...
export default function StudentHome() {
    ...
    api
      ...
      .then((list) => {
        ...
        setCurrentId((current) =>
          list.some((module) => module.id === current)
            ? current
            : (list[0]?.id ?? null),
        );
      })
      .catch((err) =>
        setError(
          err instanceof Error ? err.message : "Could not refresh your lessons",
        ),
      );
  }, [user]);
  ...
  useEffect(() => onModuleChanged(refreshModules), [refreshModules]);
  useEffect(() => onModuleDeleted(refreshModules), [refreshModules]);

  // Keep the active lesson chip visible in the horizontal scroller (matters on phones).
  useEffect(() => {
    lessonNav.current?.querySelector('[aria-current="true"]')?.scrollIntoView({
      inline: "center",
      block: "nearest",
      behavior: "smooth",
    });
  ...
  async function join(e: FormEvent) {
    ...
    try {
      ...
      setJoinMessage(
        err instanceof Error ? err.message : "Could not join this classroom",
      );
    }
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-col gap-3">
          <Eyebrow>
            {classroom
              ? `Your learning space · ${classroom.name}`
              : "Your learning space"}
          </Eyebrow>
          ...
          <p className="m-0 text-[15px] text-muted">
            Choose a lesson to see what you’ll learn next.
          </p>
        </div>
            ...
            <ol className="m-0 flex list-none gap-2 p-0">
              {modules.map((m, i) => (
                <li key={m.id}>
                  ...
                  >
                    <span
                      className={`grid size-6 place-items-center rounded-full font-mono text-[10px] ${m.id === currentId ? "bg-white/15" : TINT.mint}`}
                    >
      ...
      <section
        className={`flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between ${CARD}`}
      >
        <div className="flex flex-col gap-2">
          <Eyebrow>Join a classroom</Eyebrow>
          <Heading>Have a room code?</Heading>
          <p className="m-0 text-sm text-muted">
            Join another class with the code from your teacher.
          </p>
        </div>
