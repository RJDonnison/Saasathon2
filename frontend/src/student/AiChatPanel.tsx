...
export default function AiChatPanel({ moduleId }: { moduleId: string }) {
  ...
  async function send(q: string, requestedError?: string) {
    ...
    try {
      const { reply, highlight } = await api.aiHint({
        ...
        question: q,
        history,
        ...(code?.trim() && active ? { code, exerciseId: active.exerciseId } : {}),
  ...
  const onHelpRequest = useEffectEvent((error?: string) => void send(FIND_ERROR, error))
  useEffect(() => {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-surface">
      <header className="flex flex-none flex-col gap-2.5 border-b border-border px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className={`grid size-10 flex-none place-items-center rounded-full ${TINT.lavender}`}>
              <LightbulbIcon className="size-[18px]" />
            </span>
            <Heading variant="name">Helper</Heading>
          </div>
          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${TINT.lavender}`}>Hints only</span>
        </div>
      ...
      <p className="m-0 flex flex-none items-center gap-2 border-b border-border bg-surface-soft px-5 py-2 text-xs text-muted">
        ...
        <span className="min-w-0 truncate">{active ? `Looking at ${active.label}` : 'Click into an editor so I can see your code'}</span>
      </p>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        showHighlight({
                          ...
                          endLine: m.spot!.highlight.endLine ?? m.spot!.highlight.line,
