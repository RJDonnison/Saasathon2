...
export default function AiChatPanel() {
  ...
  async function ask(e: FormEvent) {
    ...
    try {
      // moduleId is hardcoded until ModuleView is wired to real module data.
      const { reply } = await api.aiHint({ moduleId: 'module-1', studentId: user.id, question: q })
      setMessages((m) => [...m, { from: 'ai', text: reply }])
    } catch (err) {
      setMessages((m) => [...m, { from: 'ai', text: err instanceof Error ? err.message : 'Request failed' }])
    }
  }
  ...
  return (
    <section className="rounded border bg-white p-4">
      ...
      <ul className="mb-2 space-y-1 text-sm">
        {messages.map((m, i) => (
          <li key={i} className={m.from === 'me' ? 'text-gray-900' : 'text-blue-700'}>
