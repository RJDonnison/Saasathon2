...
export default function StudentClassPage() {
  ...
  const exercisesRun = current
    ? current.exercises.filter((e) => e.runs > 0).length
    : 0;
  ...
  return (
    <>
      <ClassTopBar
        backTo="/student"
        title={classroom?.name ?? "Your class"}
        subtitle={teacher}
      />
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(290px,.95fr)]">
          <div className="flex flex-col gap-5">
            ...
            <section className={`${CARD} p-5 sm:p-6`}>
              ...
              {lessons === null ? (
                <div
                  className="mt-4 h-24 animate-pulse rounded-xl bg-surface-soft motion-reduce:animate-none"
                  aria-busy="true"
                />
              ) : current ? (
                <>
                  <h3 className="mb-1! mt-3! font-display! text-[16px]! font-semibold!">
                    {current.title}
                  </h3>
                  <p className="mb-0! text-[13px]! text-muted">
                    {STATUS_LABEL[current.status]}.{" "}
                    {plural(current.sections.length, "section")}
                    {current.exercises.length > 0 &&
                      `, ${exercisesRun} of ${plural(current.exercises.length, "code exercise")} tried`}
                    .
                  </p>
                  {introSnippet(current.content, 220) && (
                    <p className="mb-0! mt-3! text-[14px]! leading-relaxed!">
                      {introSnippet(current.content, 220)}
                    </p>
                  )}
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                    <span
                      className={`rounded-full px-3 py-1.5 text-xs font-semibold ${TINT.lavender}`}
                    >
                      Helper: hints, never answers
                    </span>
