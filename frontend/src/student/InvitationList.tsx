...
export default function InvitationList() {
  ...
  const load = useCallback(async () => {
    try {
      setInvitations(await api.myInvitations());
      setError(null);
    } catch (err) {
      setError(
  ...
  async function respond(invitation: MyInvitation, accept: boolean) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Could not answer the invitation",
      );
      void load();
    } finally {
      setBusyId(null);
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Classroom invitations"
    >
      {invitations.map((invitation) => (
        <div
          key={invitation.id}
          className={`flex flex-wrap items-center justify-between gap-3 p-4 ${CARD}`}
        >
      ...
      {error && (
        <div
          className={`flex flex-wrap items-center justify-between gap-3 rounded-xl px-4 py-3 ${TINT.peach}`}
          role="alert"
        >
