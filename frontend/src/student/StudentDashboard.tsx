...
export default function StudentDashboard() {
  ...
  const all = lessons ?? [];
  const { completedCount: done, todo, current, started } = lessonOverview(all);
  ...
  return (
    <div className="mx-auto flex max-w-[1020px] flex-col gap-6 sm:gap-7">
      ...
      {session && (
        <Link
          to={`${classPath}/live`}
          className="flex flex-col justify-between gap-5 rounded-2xl bg-ink p-6 text-white sm:flex-row sm:items-center sm:px-7 sm:py-6"
        >
          <div className="min-w-0">
            ...
            <h2 className="m-0! font-display! text-[24px]! leading-tight! font-semibold! tracking-[-0.03em]! text-white">
              {session.moduleTitle} with {teacher}
            </h2>
            <p className="mb-0! mt-2! text-sm! text-white/80">
              {session.phase === "teach"
                ? `${teacher} is teaching. Join to follow along.`
                : `Work time. Join to work on this lesson with the helper.`}
            </p>
          </div>
          <Button variant="primary" size="lg" className="sm:min-w-[132px]">
            Join lesson
          </Button>
        </Link>
      )}
      ...
      {!session && lessons !== null && (
        <div className="flex flex-col justify-between gap-5 rounded-2xl bg-ink p-6 text-white sm:flex-row sm:items-center sm:px-7 sm:py-6">
          <div className="min-w-0">
            <span
              className={`mb-3 inline-flex rounded-full px-3 py-1 text-xs font-semibold ${TINT.mint}`}
            >
              {current
                ? started
            ...
            <h2 className="m-0! font-display! text-[24px]! leading-tight! font-semibold! tracking-[-0.03em]! text-white">
              {current
                ? current.title
                : all.length
                  ? `You’ve finished every lesson in ${classroom?.name ?? "this class"}`
                  : `${classroom?.name ?? "Your class"} with ${teacher}`}
            </h2>
            <p className="mb-0! mt-2! text-sm! text-white/80">
              {current
                ? introSnippet(current.content) ||
                  `${plural(current.sections.length, "section")} to work through.`
                : all.length
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(290px,.95fr)]">
        <section className={`${CARD} overflow-hidden`}>
          <header className="flex items-center justify-between px-5 pt-5 pb-3 sm:px-6">
            ...
            <Link
              to={classPath}
              className="text-[13px]! font-semibold! text-mint-ink underline underline-offset-2"
            >
              Class page
            </Link>
          </header>
            ...
            {lessons === null ? (
              <div className="flex flex-col gap-2 py-2" aria-busy="true">
                {[0, 1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-14 animate-pulse rounded-xl bg-surface-soft motion-reduce:animate-none"
                  />
                ))}
              </div>
            ) : all.length === 0 ? (
              <p className="m-0 border-t border-border px-2 py-6 text-center text-sm text-muted">
                No lessons yet.
              </p>
            ) : (
              all.map((lesson, i) => (
                <Link
                  ...
                  className="grid grid-cols-[32px_minmax(0,1fr)_auto] items-center gap-3 border-t border-border px-2 py-3.5 hover:bg-surface-soft"
                >
                  <span className="grid size-8 place-items-center rounded-full border border-border bg-surface text-[13px] font-semibold text-muted">
                    {i + 1}
                  </span>
                  <span className="min-w-0">
                    ...
                    <span className="block text-xs text-muted">
                      ...
                      {lesson.exercises.length > 0 &&
                        `, ${plural(lesson.exercises.length, "code exercise")}`}
                    </span>
                  </span>
                  <span
                    className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${STATUS_TINT[lesson.status]}`}
                  >
          ...
          <section className={`${CARD} p-5`}>
            <header className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-muted">
                {plural(todo.length, "lesson")} left
              </span>
            </header>
            {todo.length === 0 ? (
              <p className="m-0 border-t border-border pt-3 text-[13px] text-muted">
                {all.length ? "Everything is finished." : "Nothing to do yet."}
              </p>
            ) : (
              todo.slice(0, showAll ? undefined : 4).map((item) => (
                <Link
                  key={item.id}
                  to={`${classPath}/live?lesson=${item.id}`}
                  className="block border-t border-border py-3 first:border-0"
                >
            ...
            {todo.length > 4 && (
              <button
                ...
                className="mt-2 text-[13px]! font-semibold! text-mint-ink underline underline-offset-2"
              >
                {showAll ? "Show less" : `See all ${todo.length}`}
              </button>
            )}
          </section>
          ...
          <section className={`${CARD} p-5`}>
            ...
            {announcements === null ? null : announcements.length === 0 ? (
              ...
              announcements.slice(0, 3).map((note) => (
                <article
                  key={note.id}
                  className="mt-3 border-t border-border pt-3 first:border-0"
                >
                  <p className="mb-1! text-xs! text-muted">
                    {note.authorName}, {timeAgo(note.createdAt)}
                  </p>
                  <p className="mb-0! text-[13px]! leading-relaxed! whitespace-pre-line">
                    {note.text}
                  </p>
                </article>
              ))
