...
export default function StudentWorkView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([aggregate, students, snapshots]) => {
        if (cancelled) return;
  ...
  const viewedModuleId = active?.moduleId ?? moduleId;
  useEffect(() => {
    if (!viewedModuleId || module?.id === viewedModuleId) return;
    let cancelled = false;
    setModule(null);
    api.getModule(viewedModuleId)
      .then((lesson) => {
        if (!cancelled) {
      ...
      .catch((reason) => {
        if (!cancelled)
          setError(reason instanceof Error ? reason.message : "Could not load the student's lesson");
      });
    return () => { cancelled = true; };
  }, [viewedModuleId, module?.id]);
  ...
  const activeIsCurrentModule = !!active && active.moduleId === module?.id;
  const questionId = activeIsCurrentModule
    ? active.questionId
    : module?.id === moduleId ? searchParams.get("questionId") : null;
  ...
  const section = activeIsCurrentModule
    ? module?.sections.find((item) => item.id === active.sectionId)
    : null;
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex flex-col gap-3">
          <Heading as="h1" variant="title">Connected to {studentName}</Heading>
          <p className="m-0 text-[15px] text-muted">
            Read-only view of their current lesson and saved work. Changes update as they work.
          </p>
        </div>
      ...
      {module && !question && (
        <Card title={module.title} eyebrow={section?.title ?? "Current lesson"} icon={<BookIcon className="size-[18px]" />} tint="mint" bodyClassName="flex flex-col gap-4 p-5">
          {module.content && <div className="whitespace-pre-wrap text-sm leading-relaxed text-muted"><InlineText text={module.content.replace(/^#{1,6}[ \t]+.*\n+/, "").trim()} /></div>}
          {section?.blocks.map((block) => typeof block.content === "string" ? (
            <div key={block.id} className="whitespace-pre-wrap text-sm leading-relaxed text-muted"><InlineText text={block.content} /></div>
          ) : null)}
          <p className="m-0 text-sm text-muted">{active ? `${activityLabel[active.type]} · updated ${new Date(active.createdAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : "Their current lesson is open. Their activity will appear here as they work."}</p>
        </Card>
      )}

      {question && (
        <div className="flex flex-col gap-4">
          <Card title={module?.title ?? "Current lesson"} eyebrow={section?.title ?? "Their current question"} icon={<BookIcon className="size-[18px]" />} tint="mint" bodyClassName="flex flex-col gap-3 p-5">
            {module?.content && <div className="whitespace-pre-wrap text-sm leading-relaxed text-muted"><InlineText text={module.content.replace(/^#{1,6}[ \t]+.*\n+/, "").trim()} /></div>}
            {section?.blocks.map((block) => typeof block.content === "string" ? <div key={block.id} className="whitespace-pre-wrap text-sm leading-relaxed text-muted"><InlineText text={block.content} /></div> : null)}
          </Card>
