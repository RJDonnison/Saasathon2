...
import { toUser, unwrap, type UserRow } from './rows.js';
...
//   member   — identity that has also joined a classroom (a row in `users`), which carries role + classroom
...
export async function verifyToken(token: string): Promise<AuthIdentity | null> {
  ...
  const identity: AuthIdentity = { authId: data.user.id, name: (meta?.full_name ?? meta?.name ?? fallback).trim() };
...
export async function findProfile(authId: string): Promise<User | null> {
  const row = unwrap(await supabase.from('users').select('*').eq('id', authId).maybeSingle()) as UserRow | null;
...
export const requireMember: RequestHandler = async (req, res, next) => {
  await requireIdentity(req, res, async () => {
    ...
    req.user = { userId: profile.id, role: profile.role, classroomId: profile.classroomId };
    next();
  });
};
