...
- Signing in is not enough to use the app: the user then enters a **room code + role** on the join page
  (`POST /api/auth/join`), which upserts their `users` row (`users.id` = the Supabase auth user id; name from
  their Google profile). Role and classroom are read from that row on every request — not from the token.
